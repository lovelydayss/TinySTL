...
TEST_CASE(" no-member function ( mSTL::get(...) series ) ") {

	///<- string

	std_array<string, 10> stdarray_s = {"a", "b", "c", "d", "e"};
	m_array<string, 10>   marray_s = {"a", "b", "c", "d", "e"};

	// T&

	CHECK_EQ(mSTL::get<0>(marray_s), std::get<0>(stdarray_s));
	CHECK_EQ(mSTL::get<3>(marray_s), std::get<3>(stdarray_s));
	CHECK_EQ(mSTL::get<6>(marray_s), std::get<6>(stdarray_s));
	CHECK_EQ(mSTL::get<9>(marray_s), std::get<9>(stdarray_s));

	CHECK_EQ(marray_s[0], stdarray_s[0]);
	CHECK_EQ(marray_s[3], stdarray_s[3]);
	CHECK_EQ(marray_s[6], stdarray_s[6]);
	CHECK_EQ(marray_s[9], stdarray_s[9]);
	...
	std::get<0>(stdarray_s) = "c";
	std::get<3>(stdarray_s) = "d";
	std::get<6>(stdarray_s) = "e";
	std::get<9>(stdarray_s) = "f";
	...
	const m_array<string, 10>   c_marray_s = {"a", "b", "c", "d", "e"};

	CHECK_EQ(mSTL::get<0>(c_marray_s), std::get<0>(c_stdarray_s));
	CHECK_EQ(mSTL::get<3>(c_marray_s), std::get<3>(c_stdarray_s));
	CHECK_EQ(mSTL::get<6>(c_marray_s), std::get<6>(c_stdarray_s));
	CHECK_EQ(mSTL::get<9>(c_marray_s), std::get<9>(c_stdarray_s));

	// const T&&

	CHECK_EQ(std::move(std::move(mSTL::get<0>(marray_s))),
	         std::move(std::move(std::get<0>(stdarray_s))));
	CHECK_EQ(std::move(std::move(mSTL::get<3>(marray_s))),
	         std::move(std::move(std::get<3>(stdarray_s))));
	CHECK_EQ(std::move(std::move(mSTL::get<6>(marray_s))),
	         std::move(std::move(std::get<6>(stdarray_s))));
	CHECK_EQ(std::move(std::move(mSTL::get<9>(marray_s))),
	...
	m_array<int, 10>   marray_i = {1, 2, 3, 4, 5};

	// T&

	CHECK_EQ(mSTL::get<0>(marray_i), std::get<0>(stdarray_i));
	CHECK_EQ(mSTL::get<3>(marray_i), std::get<3>(stdarray_i));
	CHECK_EQ(mSTL::get<6>(marray_i), std::get<6>(stdarray_i));
	CHECK_EQ(mSTL::get<9>(marray_i), std::get<9>(stdarray_i));

	CHECK_EQ(marray_i[0], stdarray_i[0]);
	CHECK_EQ(marray_i[3], stdarray_i[3]);
	CHECK_EQ(marray_i[6], stdarray_i[6]);
	CHECK_EQ(marray_i[9], stdarray_i[9]);
	...
	std::get<0>(stdarray_i) = 9;
	std::get<3>(stdarray_i) = 10;
	std::get<6>(stdarray_i) = 11;
	std::get<9>(stdarray_i) = 12;
	...
	const m_array<int, 10>   c_marray_i = {1, 2, 3, 4, 5};

	CHECK_EQ(mSTL::get<0>(c_marray_i), std::get<0>(c_stdarray_i));
	CHECK_EQ(mSTL::get<3>(c_marray_i), std::get<3>(c_stdarray_i));
	CHECK_EQ(mSTL::get<6>(c_marray_i), std::get<6>(c_stdarray_i));
	CHECK_EQ(mSTL::get<9>(c_marray_i), std::get<9>(c_stdarray_i));

	// const T&&
	CHECK_EQ(std::move(std::move(mSTL::get<0>(marray_i))),
	         std::move(std::move(std::get<0>(stdarray_i))));
	CHECK_EQ(std::move(std::move(mSTL::get<3>(marray_i))),
	         std::move(std::move(std::get<3>(stdarray_i))));
	CHECK_EQ(std::move(std::move(mSTL::get<6>(marray_i))),
	         std::move(std::move(std::get<6>(stdarray_i))));
	CHECK_EQ(std::move(std::move(mSTL::get<9>(marray_i))),
	         std::move(std::move(std::get<9>(stdarray_i))));
